...
2. **Data model + persistence**
   ...
   - Persistence: write only `*.booru.json` for edits (tags, rating, notes).
...
4. **Core operations**
   - Query/filter: by tags, rating, date, source site, etc.
   - Edit operations: add/remove tags, rating, notes.
   - Export or write-back strategy (optional) for metadata edits.

5. **TUI (ratatui)**
   - Search/filter panel, list view, detail view, preview panel.
   - Keybindings for tagging and rating.
